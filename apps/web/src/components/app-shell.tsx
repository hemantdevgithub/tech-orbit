...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const sidebar = (
    <aside className="w-64 shrink-0 bg-forest-800 text-cream-100 flex flex-col h-screen sticky top-0">
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-6">
        ...
        {user && (
          <div className="px-4">
            <Link
              href={`/users/${user.id}`}
              className="flex items-center gap-3 p-3 rounded-lg hover:bg-forest-700/60 transition-colors"
            >
              <div className="w-10 h-10 rounded-full bg-mint-300 text-forest-800 text-sm font-bold flex items-center justify-center shrink-0">
      ...
      {/* Footer */}
      <div className="shrink-0 border-t border-forest-700/60 p-3">
        <button
          onClick={() => void logout()}
          className="w-full text-left px-3 py-2 rounded-md text-sm text-sage-300 hover:text-cream-100 hover:bg-forest-700/60 transition-colors"
        >
          Sign out
        </button>
      </div>
