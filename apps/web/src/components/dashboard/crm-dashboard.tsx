    ...
    <div className="space-y-6">
      ...
      {!loading && (
        <StatStrip
          ...
          stats={[
            { label: "Attributed requirements", value: openReqs, href: "/techforce/requirements" },
            { label: "Pending attributions", value: pendingAttributions.length, href: "/techforce/requirements" },
        ...
        <div className="bg-white rounded-xl border border-sage-200 divide-y divide-sage-100">
          {loading ? (
            ...
            <div className="px-5 py-8 text-center">
              ...
              <Link href="/techforce/requirements" className="text-xs text-forest-700 hover:underline">Browse requirements to claim →</Link>
            </div>
          ) : (
            recentAttributions.map((a) => (
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4 px-4 sm:px-5 py-3 sm:py-3.5">
                ...
                <Link href={`/techforce/requirements/${a.requirementId}`} className="text-xs text-forest-700 hover:underline shrink-0">View →</Link>
              </div>
            ))
          )}
        </div>
