      ...
      <div className="flex h-screen items-center justify-center">
        <Card>
          <CardBody>
            ...
            <Link href={`/techforce/interviews/${params?.id ?? ""}`} className="mt-2 block text-sm text-forest-700 hover:underline">
              ← Back to interview
            </Link>
          </CardBody>
        </Card>
    ...
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4">
        ...
        {(user?.id === iv.scheduledByUserId || user?.id === iv.interviewerUserId) && (
          <Link href={`/techforce/interviews/${iv.id}/scorecard`}>
...
function CallTopBar({ iv }: { iv: InterviewResponse }) {
  ...
  return (
    <div className="flex items-center justify-between bg-forest-900 px-4 py-2 text-cream-100">
      ...
      <Link href={`/techforce/interviews/${iv.id}`} className="text-xs text-sage-400 hover:text-cream-100">
        ← Details
      </Link>
    </div>
  );
