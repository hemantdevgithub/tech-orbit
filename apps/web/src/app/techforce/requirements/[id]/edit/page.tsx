...
export default function EditRequirementPage() {
    ...
    async function run(): Promise<void> {
      ...
      try {
        ...
        if (req.status !== "DRAFT") {
          ...
          router.replace(`/techforce/requirements/${req.id}`);
          return;
        }
  ...
  async function onSubmit(data: FormData): Promise<void> {
    ...
    try {
      ...
      router.push(`/techforce/requirements/${params.id}`);
    } catch (err) {
  ...
  return (
    <div>
      <div className="mb-4">
        <Link
          href={`/techforce/requirements/${params?.id}`}
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="mt-6 flex justify-end gap-2">
          <Link href={`/techforce/requirements/${params?.id}`}>
