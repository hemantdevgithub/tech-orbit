...
function contextLinkFor(contextType: string, contextId: string): string | null {
  switch (contextType) {
    case "PLACEMENT":
      return `/placements/${contextId}`;
    case "COMMISSION_PAYOUT":
      return `/payouts/${contextId}`;
    case "TIMESHEET":
      return `/timesheets?open=${contextId}`;
    case "INVOICE":
      return `/invoices/${contextId}`;
    default:
      return null;
  }
}
...
export default function DisputeDetailPage() {
  ...
  async function submitResolve() {
    ...
    try {
      ...
      router.push("/admin/disputes?resolved=1");
    } catch (err) {
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        <Link href="/admin/disputes" className="text-sm text-forest-700 hover:underline">
          ← All disputes
        </Link>
