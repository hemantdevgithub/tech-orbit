...
export async function setup(): Promise<void> {
  ...
  if (!(await dockerAvailable())) {
    ...
    console.warn("[payments-setup] Docker unavailable — integration tests will be skipped.");
    return;
  }
  ...
  const url = `postgresql://test:test@${container.getHost()}:${container.getMappedPort(5432)}/payments_test`;
