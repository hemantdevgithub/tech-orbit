...
export async function setup(): Promise<void> {
  ...
  if (!(await dockerAvailable())) {
    ...
    console.warn("[rating-setup] Docker unavailable — integration tests will be skipped.");
    return;
  }

  container = await new PostgreSqlContainer("postgres:16-alpine")
    .withDatabase("rating_test")
    .withUsername("test")
    .withPassword("test")
    .start();

  const url = `postgresql://test:test@${container.getHost()}:${container.getMappedPort(5432)}/rating_test`;
